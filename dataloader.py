...
class VisDialDataset(Dataset):
    ...
    def __init__(self, args, subsets):
        ...
        if args.overfit:
            self.data[dtype + '_img_fv'] = self.data[dtype + '_img_fv'][110000:120000]
            self.data[dtype + '_img_fnames'] = self.data[dtype + '_img_fnames'][110000:120000]

        self.num_data_points = {}
        for dtype in subsets:
            self.num_data_points[dtype] = len(self.data[dtype + '_img_fv'])
            if dtype == 'train':
                self.num_data_points[dtype] = int(self.num_data_points[dtype])

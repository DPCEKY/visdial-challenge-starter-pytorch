...
class CompactBilinearPooling3D(nn.Module):
    """
    Compute compact bilinear pooling over two bottom inputs.
    Args:
        output_dim: output dimension for compact bilinear pooling.
    ...
    def __init__(self, input_dim1, input_dim2, input_dim3, output_dim,
        ...
        super(CompactBilinearPooling3D, self).__init__()
        self.input_dim1 = input_dim1
        self.input_dim2 = input_dim2
        ...
        print('*' * 20)
        print('using CompactBilinearPooling3D version')
        print('*' * 20)
        if rand_h_1 is None:
            np.random.seed(1)
            rand_h_1 = np.random.randint(output_dim, size=self.input_dim1)
    ...
    def forward(self, bottom1, bottom2, bottom3):
        ...
        bottom1: 1st input, 4D Tensor of shape [batch_size, round_len, input_dim1].
        bottom2: 2nd input, 4D Tensor of shape [batch_size, round_len, input_dim2].
        bottom3: 3nd input, 4D Tensor of shape [batch_size, round_len, input_dim3].
        """
        assert bottom1.size(2) == self.input_dim1 and \
            bottom2.size(2) == self.input_dim2 and \
            bottom3.size(2) == self.input_dim3

        batch_size, round_len, _ = bottom1.size()

        bottom1_flat = bottom1.contiguous().view(-1, self.input_dim1)
        bottom2_flat = bottom2.contiguous().view(-1, self.input_dim2)
        bottom3_flat = bottom3.contiguous().view(-1, self.input_dim3)
        ...
        sketch_3_fft = torch.rfft(sketch_3, 1)
        fft3_real = sketch_3_fft[:, :, 0]
        fft3_imag = sketch_3_fft[:, :, 1]

        dialog_real = fft1_real.mul(fft2_real) - fft1_imag.mul(fft2_imag)
        dialog_imag = fft1_real.mul(fft2_imag) + fft1_imag.mul(fft2_real)
        # print('fft_product_real.shape = {}, fft_product_imag.shape = {}'.format(fft_product_real.shape, fft_product_imag.shape))

        fft_product_real = dialog_real.mul(fft3_real) - dialog_imag.mul(fft3_imag)
        fft_product_imag = dialog_real.mul(fft3_imag) + dialog_imag.mul(fft3_real)
...
class RvAEncoder(nn.Module):
    def __init__(self, config, vocabulary):
        ...
        # multimodel 3D compact bilinear pooling for outer prod
        # self.dialog_mcb = CompactBilinearPooling(300, 1024, 4096)
        # self.visual_mcb = CompactBilinearPooling(2048, 4096, 24576)
        # self.mcb_linear = nn.Linear(24576, 3372)

        # self.mcb = CompactBilinearPooling(2048, 1324, 3372)

        self.mcb = CompactBilinearPooling3D(300, 1024, 2048, 3372)
    ...
    def forward(self, batch, return_att=False):
        ...
        # print('ques_ans_feat.size() = {}'.format(ques_ans_feat.size())) 300
        # print('hist_ans_feat.size() = {}'.format(hist_ans_feat.size())) 1024
        
        # dialog_vector = self.dialog_mcb(ques_ans_feat, hist_ans_feat)
        # fused_mcb_vector = self.visual_mcb(img_ans_feat, dialog_vector)
        # fused_vector = self.mcb_linear(fused_mcb_vector)

        fused_vector = self.mcb(ques_ans_feat, hist_ans_feat, img_ans_feat)
